...
const NewGroupModal = () => {
  ...
  const [selectedUsers, setSelectedUsers] = React.useState<IUSER[]>(user ? [user] : []);
  const [isLoading, setIsLoading] = React.useState<boolean>(false);
  const [isInvalid, setIsInvalid] = React.useState<boolean>(false);

  const [groupName, setGroupName] = React.useState<string>("");
  const [groupDescription, setGroupDescription] = React.useState<string>("");
  const [groupConfirmNumber, setGroupConfirmNumber] = React.useState<string>("");
  const api = useAPI();
  ...
  React.useEffect(() => {
    ...
    const _contract = new Contract(Factory_ADDRESSES[chainId], FACTORY_ABI, signer);
    setContract(_contract);
  }, [address, chainId, signer]);

  useEffect(() => {
    setPreview("https://i.ibb.co/5kVNC9N/group1.jpg");
  ...
  const _submitRegister = async () => {
    ...
    try {
      ...
      const tx = await contract.createGroup(groupName, groupDescription, memberAddresses, Number(groupConfirmNumber));
      await tx.wait();
      const numberOfCreators = await contract.numberOfCreators();
      const _group_Address = await contract.getCreatorGroupAddress(Number(numberOfCreators)-1);
    ...
    <>
      <div className="z-100 font-Maxeville">
        ...
        <div className="joinModal drop-shadow-lg">
          ...
          {addMemberModalState && <AddMemberModal addSelectedUsers={addSelectedUsers} />}
          <div className="ps-[20px] pe-[10px] py-[20px] rounded-lg">
            ...
            <div className="max-h-[678px] overflow-auto scrollbar">
              <div className="flex justify-center items-center mt-2">
                <div className="content-card border bg-gray-200 relative w-1/2 ">
                  <Image
                    ...
                    alt=""
                  />
                  <label htmlFor="avatar" className="content-card-menu hidden justify-center gap-1 flex-col items-center absolute top-0 w-full h-full bg-chocolate-main/50">
                    <div className="border bg-[#322A44] text-white rounded-full pt-2 pb-2 pl-3 pr-3 w-1/2 text-lg text-center cursor-pointer">
                      UPLOAD
                    </div>
                ...
                {selectedUsers.map((index, key) => (
                  <div key={key}>
                    <Image
                      ...
                      alt="avatar"
                    />
                    <h2 className="text-center">{index.name === user?.name ? "You" : index.name}</h2>
                  </div>
                ))}
                <div
              ...
              <div className="flex justify-center items-center mt-5 mb-3">
                <button className="border bg-[#322A44] text-white rounded-full pl-4 pr-4 w-[380px] text-lg flex items-center justify-center text-center" onClick={handleSubmit}>
                  { isLoading ?
                <>
