...
const BidGroupModal = ({ nftData, groupAddress, groupId, getData }: BidGroupModalInterface) => {
    ...
    const handleBidClick = async () => {
        ...
        try {
            ...
            if (Number(nftData.auctiontype) === 0) {
                if (Number(bidAmount) <= Number(nftData.currentprice)) throw "You must bid higher than now";
                const tx1 = await usdc_contract.approve(Marketplace_ADDRESSES[chainId], BigInt(Number(bidAmount) * 1e18));
                await tx1.wait();
                const tx = await contract.makeBidToEnglishAuction(BigInt(nftData.listednumber), BigInt(Number(bidAmount) * 1e18));
                await tx.wait();
                const auction_data = await contract.englishAuctions(BigInt(nftData.listednumber));
                ...
                await api.post("/api/updateNft", {
                    id: nftData.id, owner: nftData.owner, status: "list", auctionType: nftData.auctiontype, initialPrice: nftData.initialprice,
                    salePeriod: nftData.saleperiod, currentPrice: currentPrice, currentBidder: user.name, reducingRate: nftData.reducingrate ? nftData.reducingrate : 0,
                ...
                const tx1 = await usdc_contract.approve(Marketplace_ADDRESSES[chainId], BigInt(Number(bidAmount) * 1e18));
                await tx1.wait();
                const tx = await contract.makeBidToOfferingSale(BigInt(nftData.listednumber), BigInt(Number(bidAmount) * 1e18));
                await tx.wait();
                ...
                const offering_number = await _group_contract.getNumberOfSaleOfferingTransaction() ;
                await api.post("/api/addOffering", {
                    groupId: groupId, buyer: user.name, nftId: nftData.id, confirm_member: JSON.stringify([]), price: bidAmount, transactionId:(Number(Number(offering_number) - 1)).toString()
                })
            }
            ...
            <div className="joinModal drop-shadow-lg">
                ...
                <div className="p-5  rounded-lg flex-col justify-between">
                    ...
                    <div className="flex justify-center items-center mt-5 mb-5">
                        <button className="border bg-[#322A44] text-white rounded-full pl-4 pr-4 w-[380px] text-lg text-center flex items-center justify-center" onClick={handleBidClick}>
                            {isLoading ?
                                <>
