...
import { IGROUP, IUSER, INFT, IOFFER_TRANSACTION, IDIRECTOR_TRANSACTION } from "@/types";
...
const ShareGroupProfile = ({ params }: { params: { id: string } }) => {
  ...
  const [offerTransactions, setOfferTransactions] = useState<IOFFER_TRANSACTION[]>([]);
  const [directorTransactions, setDirectorTransactions] = useState<IDIRECTOR_TRANSACTION[]>([]);
  ...
  const getNFTData = async () => {
    const result1 = await api.post('/api/getNftByGroupAndStatus', { id: params.id, status: "sold" });
    console.log("result1", result1.data);
    setSoldNfts(result1.data);
    const result2 = await api.post('/api/getNftByGroupAndStatus', { id: params.id, status: "list" });
    setListedNfts(result2.data);
    console.log("result2", result2.data);
    const result3 = await api.post('/api/getNftByGroupAndStatus', { id: params.id, status: "mint" });
  ...
  const getOffer_nfts = async () => {
    const _nfts = listedNfts.filter((item: INFT) => offerTransactions.map((_offer: IOFFER_TRANSACTION) => _offer.nftid).includes(item.id));
    console.log({ _nfts });
    setOfferNfts(_nfts);
  }
  ...
  const [withdrawFromMarketplaceAmount, setWithdrawFromMarketplace] = useState<string>("");
  ...
  const [marketplaceContract, setMarketplaceContract] = useState<Contract | undefined>(undefined);
  ...
  const offeringConfrimHandle = async (item: IOFFER_TRANSACTION) => {
    try {
      ...
      setIsLoading(true);
      const tx = await contract.confirmOfferingSaleTransaction(BigInt(item.transactionid), true);
      await tx.wait();
      const confirm_member = item.confirm_member;
      confirm_member.push({ "id": user.id })
      await api.post("/api/updateOffering", { id: item.id, confirm_member: JSON.stringify(confirm_member) });
      getNFTData();
  ...
  const offeringExecuteHandle = async (item: IOFFER_TRANSACTION, item_nft: INFT) => {
    try {
      if (!contract) throw "no contract";
      if (!chainId) throw "Invalid chain id";
      if (!user) throw "You must sign in";
      setIsLoading(true);
      const tx = await contract.excuteOfferingSaleTransaction(BigInt(item.transactionid));
      await tx.wait();
      await api.post("/api/removeOffering", { id: item.nftid });

      await api.post('/api/updateNft', {
        id: item_nft?.id, owner: item.buyer, status: 'sold', auctionType: item_nft?.auctiontype, initialPrice: item_nft?.initialprice,
        salePeriod: item_nft?.saleperiod, currentPrice: item_nft?.currentprice, currentBidder: item_nft?.currentbidder, reducingRate: item_nft?.reducingrate, listedNumber: item_nft.listednumber
      })
      getMyGroupData();
      getNFTData();
    } catch (error: any) {
  ...
  const directorConfrimHandle = async (item: IDIRECTOR_TRANSACTION) => {
    try {
      ...
      if (!user) throw "You must sign in";
      setIsLoading(true);
      const tx = await contract.confirmDirectorSettingTransaction(BigInt(item.transaction_id), true);
      await tx.wait();
      const confirm_member = item.confirm_member;
      confirm_member.push({ "id": user.id })
      await api.post("/api/updateDirector", { id: item.id, confirm_member: JSON.stringify(confirm_member) });
      getNFTData();
  ...
  const directorExecuteHandle = async (item: IDIRECTOR_TRANSACTION) => {
    try {
      ...
      if (!user) throw "You must sign in";
      setIsLoading(true);
      const tx = await contract.excuteDirectorSettingTransaction(BigInt(item.transaction_id));
      await tx.wait();
      await api.post("/api/removeDirector", { id: item.new_director });
      await api.post("/api/updateGroupDirector", { id: myGroupData?.id, director: item.new_director });
      getMyGroupData();
      getNFTData();
    } catch (error: any) {
      if (String(error.code) === "ACTION_REJECTED") {
  ...
  const leaveGroupHandle = async () => {
    try {
      ...
      await tx.wait();
      const _member = myGroupData?.member.filter((_user) => _user.id !== user.id);

      await api.post("/api/updateGroupMember", { id: myGroupData?.id, member: JSON.stringify(_member) });
      getMyGroupData();
      getNFTData();
    } catch (error: any) {
      if (String(error.code) === "ACTION_REJECTED") {
  ...
  const getBalancesForWithdraw = async () => {
    ...
    console.log("(Number(Number(totalEarningAmount) / 1e18)).toString(), ", (Number(Number(totalEarningAmount) / 1e18)).toString());
    await api.post("/api/updateEarning", { id: myGroupData?.id, earning: (Number(Number(totalEarningAmount) / 1e18)).toString() })
    console.log("success here it is");
    if (!marketplaceContract) return;

    const withdrawMarketplaceBalance = await marketplaceContract.balanceOfSeller(myGroupData?.address);
    setWithdrawFromMarketplace(Number(Number(withdrawMarketplaceBalance) / 1e18).toString());
  ...
  const suggestDirectorSetting = async (_num: number) => {
    try {
      ...
      console.log("members:", members_in_group, " next member:", members_in_group1, " current director ", currentDirector);
      console.log("dfafds", members[_num].wallet);

      const tx = await contract.submitDirectorSettingTransaction(members[_num].wallet);
      await tx.wait();
      const transaction_id = await contract.getNumberOfCandidateTransaction();
      await api.post("/api/addDirector", { groupid: myGroupData?.id, new_director: members[_num].id, suggester: user?.id, confirm_member: JSON.stringify([]), transaction_id: Number(Number(transaction_id) - 1).toString() });
      getNFTData();
  ...
  return (
    <>
      {mintModalState && (
        avatar &&
        <MintModal groupAddress={myGroupData ? myGroupData.address : ""} groupId={parseInt(params.id)} mintAvatar={mintAvatar} avatarFile={avatar}
          deleteContent={deleteContent} uploadId={uploadId} getNFTData={getNFTData} />
      ...
      <div className="pt-[100px] h-full">
          ...
          <div>
            <div className="gap-4 grid xl:grid-cols-2 lg:grid-cols-1 xl:w-[50%] xl:min-w-[920px] xs:w-full xs:h-full">
              ...
              <div className="group_info mt-5">
                {
                  members && myGroupData &&
                  <GroupDescription users={members} myGroupData={myGroupData} totalEarning={totalEarning} />
                }
              </div>
            </div>
          </div>
                  ...
                  <div className="">
                    <div className="flex items-baseline space-x-4">
                      ...
                      <a
                        ...
                        className="border-b-2 border-transparent hover:border-gray-400 px-3 py-2 text-lg"
                      >
                        OFFERS ({offerTransactions ? offerTransactions.length : "0"})
                      </a>
                      <a
                        onClick={() => {
        ...
        <div className="grouppage_container font-Maxeville">
          <div className="flex justify-between text-xl mt-5" id="nfts">
            <div>NFTs ({(myGroupData?.mintnumber) ? myGroupData?.mintnumber : "0"})</div>
          ...
          <div className="grid grid-cols-6 gap-4 mt-5 xl:grid-cols-6 md:grid-cols-4 sm:grid-cols-3 xs:grid-cols-2 mb-[50px]">
            {soldNfts.map((item, index) => (
              ...
              >
                <NftCard name={""} avatar={item.avatar} collectionName={item.collectionname} collectionId={Number(item.collectionid)} seen={200} favorite={20} price={Number(item.currentprice)} />
              </div>
            ))}
          </div>
          <Split_line />
          ...
          <div className="flex justify-between text-lg" id="offers">
            <div>OFFERS ({offerTransactions.length ? offerTransactions.length : "0"})</div>
            <div className="border-b-2 border-indigo-500"></div>
          </div>
          <div className="justify-start gap-2 mt-3 lg:grid lg:grid-cols-2 xs:grid xs:grid-cols-1">
            {offerTransactions.length && myGroupData && members?.length && offerNfts.length && offerTransactions.map((item, key) => (
              <div key={key}>
                  ...
                  <div className="flex flex-col">
                    ...
                    <div className="xs:grid xs:grid-cols-1 lg:grid lg:grid-cols-1 xl:grid xl:grid-cols-3">
                      <div className="flex me-[5px]">
                        ...
                        <div className="ms-[5px]">{offerTransactions[key].price} USDC</div>
                      </div>
                      <div className="flex">
                    ...
                    <div className="text-gray-400 mt-5 text-md">
                      CONFIRMED BY {offerTransactions[key].confirm_member.length}/{myGroupData.member.length}
                    </div>
                    <div className="my-[20px]">
                      {renderAvatar(
                        members.filter((item: IUSER) => offerTransactions[key].confirm_member.map((_id: any) => _id.id).includes(item.id))
                      )}
                    </div>
                    <div className="flex flex-col w-full">
                      <button className="border border-black rounded-full pl-4 pr-4 w-[200px] text-[18px] mb-[5px]" onClick={() => offeringConfrimHandle(offerTransactions[key])}>
                        CONFIRM
                      </button>
                      <button className="border border-black rounded-full pl-4 pr-4 w-[200px] text-[18px]" onClick={() => offeringExecuteHandle(offerTransactions[key], offerNfts[key])}>
                        EXECUTE
                      </button>
                    </div>
                  </div>
                  ...
                  <div className="content-card relative ">
                    ...
                    />
                    {
                      ...
                      <div className="content-card-menu hidden justify-center gap-1 flex-col items-center absolute top-0 w-full h-full bg-chocolate-main/80 rounded-lg">
                        ...
                        <button className="border bg-[#EF2121] text-white rounded-full w-[75%] text-[18px] h-[30px]" onClick={() => deleteContent(item)}>
                          DELETE
                        </button>
                      </div>
                    }
          ...
          <div className="flex justify-center items-center mt-5">
            <label htmlFor="avatar" className="border bg-[#322A44] text-white rounded-full pl-4 pr-4 w-[380px] text-lg cursor-pointer text-center">
          ...
          <div className="grid grid-cols-8 mt-5 xl:grid-cols-8 md:grid-cols-5 sm:grid-cols-4 xs:grid-cols-3 m-auto">
              ...
              >
                {
                  ...
                  <>
                    <div className="aspect-square rounded-full">
                      <Image
                        src={members?.filter((_user: IUSER) => _user.id.includes(myGroupData?.director))[0].avatar}
                    ...
                    <div className="mt-3 justify-center">
                      <p className="flex justify-center">{members?.filter((_user: IUSER) => _user.id.includes(myGroupData?.director))[0].name} (Current Director)</p>
                    </div>
                  </>
                }
              </div>
            ...
            {members && members.map((item, index) => (
              <>
                <div className={`flex flex-col ${item.id === (myGroupData?.director) && 'hidden'} `}>
                  <div
                    key={index}
                    className={`flex flex-col items-center justify-center cursor-pointer rounded-lg m-2 ${selected === index ? "border border-chocolate-main/50" : "border border-white"}`}
                  ...
                  <button className={`border bg-[#322A44] text-white rounded-full text-lg text-center ${item.id === myGroupData?.director ? "hidden" : ""} `} onClick={() => suggestDirectorSetting(index)}>
                    SUGGEST
                  </button>
                </div>
              </>
          ...
          <div className="flex justify-between text-lg mt-5" id="offers">
            <div>SUGGESTED DIRECTORS ({directorTransactions.length ? directorTransactions.length : "0"})</div>
            <div className="border-b-2 border-indigo-500"></div>
          </div>
          {
            directorTransactions && directorTransactions.length && myGroupData && members?.length && members &&
            <div className="justify-start gap-2 mt-3 lg:grid lg:grid-cols-2 xs:grid xs:grid-cols-1">
              {directorTransactions.map((item, key) => (
                <div key={key}>
                  <div className="min-w-[50%] flex mt-[30px] gap-5">
                    <div>
                      {
                        directorTransactions && members && <Image
                          src={members?.filter((_user: IUSER) => _user.id.includes(directorTransactions[key].new_director))[0].avatar}
                          className="aspect-square object-cover rounded-lg"
                          width={300}
                          height={300}
                          alt="offer"
                    ...
                    <div className="flex flex-col justify-between">
                      ...
                      <div className="mb-[5px]">{members?.filter((_user: IUSER) => _user.id.includes(directorTransactions[key].suggester))[0].name} <span className="text-gray-400">SUGGESTED</span></div>

                      {
                        members && directorTransactions && <div className="text-gray-400 mt-5 text-md">
                          CONFIRMED BY {directorTransactions[key].confirm_member.length ? directorTransactions[key].confirm_member.length : "0"}/{myGroupData.member.length}
                        </div>

                      }
                      <div className="my-[20px]">
                        {renderAvatar(
                          members.filter((_user: IUSER) => directorTransactions[key].confirm_member.map((_id: any) => _id.id).includes(_user.id))
                        )}
                      </div>
                      <div className="flex flex-col w-full">
                        <button className="border border-black rounded-full pl-4 pr-4 w-[200px] text-[18px] mb-[5px]" onClick={() => directorConfrimHandle(directorTransactions[key])}>
                          CONFIRM
                        </button>
                        <button className="border border-black rounded-full pl-4 pr-4 w-[200px] text-[18px]" onClick={() => directorExecuteHandle(directorTransactions[key])}>
                          EXECUTE
                        </button>
                      </div>
                    </div>
          ...
          <div className="mt-5 text-lg lg:flex">
            ...
            <div className="lg:block xs:flex xs:justify-center xs:mt-5 lg:mt-0 lg:ms-[25px]">
              <button onClick={withdrawFromGroup} className="border border-black rounded-full pl-4 pr-4 w-[190px] text-lg hover:bg-chocolate-main hover:text-white transition-all">
                Withdraw
              </button>
            </div>
          </div>
          {
            ...
            <div className="mt-5 text-lg lg:flex">
              <div className="flex gap-5">
                <div className="flex items-center h-[32px]">AMOUNT</div>
                <div className="flex border-2 border-black items-center justify-center pl-5 pr-5 rounded-lg text-gray-400">
                  {withdrawFromMarketplaceAmount ? withdrawFromMarketplaceAmount : "0"}
                </div>
                <div className="flex items-center h-[32px]">USDC</div>
              </div>

              <div className="lg:block xs:flex xs:justify-center xs:mt-5 lg:mt-0 lg:ms-[25px]">
                <button onClick={withdrawFromMarketplace} className="border border-black rounded-full pl-4 pr-4 w-[300px] text-lg hover:bg-chocolate-main hover:text-white transition-all">
          ...
          <div className="flex justify-center items-center mt-5">
            <button onClick={leaveGroupHandle} className="border bg-[#FF0000] text-white rounded-full pl-4 pr-4 w-[380px] text-lg">
              LEAVE THIS GROUP
            </button>
          </div>
          <Split_line />
